...
{
    if (sideFaderMode() != FaderMode::Crossfade)
        return;

    bool primaryTop = m_primaryTop;
    if (m_sideFaderLevel == 0)
        primaryTop = false;
    else if (m_sideFaderLevel == 100)
        primaryTop = true;

    if (primaryTop == m_primaryTop)
        return;

    m_primaryTop = primaryTop;
